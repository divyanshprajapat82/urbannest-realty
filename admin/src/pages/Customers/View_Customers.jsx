...
export default function View_Customers() {
    ...
    let getCustomers = () => {
        axios.get(`${VITE_APIPATH}customers/view`)
            .then((res) => res.data)
            .then((finalData) => {
                console.log(finalData.data);
                setHappyCustomers(finalData.data)
                setStaticPath(finalData.staticPath)
            })
    }

    let deletePropertyType = (id) => {
        axios.delete(`${VITE_APIPATH}customers/delete/${id}`)
            .then((res) => res.data)
            .then((finalData) => {
                if (finalData.status) {
                    ...
                    <table className='w-full '>
                        <thead className='bg-[#F1F1F1] border-b border-[#00000026]'>
                            <tr>
                                ...
                                <th className='p-3 w-[20%]'>Message</th>
                        ...
                        <tbody>
                            {happyCustomers.map((items, index) => (
                                <tr>
                                    <th><input type="checkbox" className='ml-1' /></th>
                                    <th className='p-3 text-center'>{index + 1}</th>
                                    <th className='p-3 text-center'>{items.customersName}</th>
