...
export default function View_Localities() {
    ...
    let deleteLocalities = (id) => {

        axios.delete(`${VITE_APIPATH}localities/delete/${id}`)
            .then((res) => res.data)
            .then((finalData) => {
                if (finalData.status) {
    ...
    // let SingleLocalities = (id) => {

    //     axios.delete(`${VITE_APIPATH}localities/view/${id}`)
    //         .then((res) => res.data)
    //         .then((finalData) => {
    //             if (finalData.status) {
    //                 toast.success(finalData.msg)
                        ...
                        <tbody>
                            {localitiesList.map((items, index) => (
                                <tr>
                                    ...
                                    <th className='p-3 text-center'>{items.name}</th>
                                    <td className='p-3 text-center'>{items.city}</td>
                                    <td className='p-3 text-center '>{items.description ? items.description : "-----"}</td>
                                    ...
                                    <td className=" p-3 text-center ">
                                        <div className='flex justify-center'>
                                            <Link to={`/add-localities/${items._id}`}>
                                                <button className="bg-[#F1F1F1] text-[#000] border border-[#00000025] p-3 rounded-l-[10px] cursor-pointer">
                                                    <FaRegEdit />
                                                </button>
                                            </Link>
                                            <button onClick={() => deleteLocalities(items._id)} className="bg-[#F1F1F1] text-[#f00] border border-[#00000025] p-3 rounded-r-[10px] cursor-pointer">
                                                <RiDeleteBin6Line />
                                            </button>
                                        </div>
                                    </td>
